...
JbdBms::JbdBms(int rx, int tx){
  static SoftwareSerial swSerial(rx, tx);
  m_port = &swSerial;
  m_hwserial = false;
  static_cast<SoftwareSerial*>(m_port)->begin(9600);
}
...
float JbdBms::getBalanceCapacity() {
  return m_balanceCapacity;
}

float JbdBms::getRateCapacity() {
  return m_rateCapacity;
}
...
void JbdBms::setMosfet(uint8_t mosData) {
  flush();
  uint8_t reqMessage[] = {0xDD, 0x5A, 0xE1, 0x02, 0x00, mosData, 0x00, 0x00, 0x77};
  uint16_t checkSumCompute = computeCrc16JbdChina(reqMessage);
  reqMessage[6] = (checkSumCompute >> 8) & 0xFF;
  reqMessage[7] = checkSumCompute & 0xFF;
  m_port->write(reqMessage, 9);
  flush();
}

void JbdBms::parseReqBasicMessage(uint8_t * t_message) {
  m_voltage = ((float)convertTwoIntsToUint16(t_message[4], t_message[5])) * 10;
  m_current = ((float)convertTwoIntsToInt16(t_message[6], t_message[7])) * 10;
  m_balanceCapacity = ((float)convertTwoIntsToInt16(t_message[8], t_message[9])) * 10;
  m_rateCapacity = ((float)convertTwoIntsToInt16(t_message[10], t_message[11])) * 10;
...
bool JbdBms::checkCheckSumRecieve(uint8_t *t_message)
{
  ...
  checkSumCompute = computeCrc16JbdChina(t_message);
...
uint16_t JbdBms::computeCrc16JbdChina(uint8_t *puchMsg)
{
  ...
  for (int i = 2; i < lengthData + 4; i++)
    summa = summa + puchMsg[i];

  uint16_t checkSum = (summa - 1) ^ 0xFFFF;
  return checkSum;
}
